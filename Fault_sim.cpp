...
#define CONSERVATIVE_MODE 0 // 1: Conservavie mode, 0: Restrained mode
//configuration over

// 공인인증을 위한 주석

// 상기 Configuration은 x4 chip DDR5의 sub-channel의 configuration입니다.
// 하나의 sub-channel은 8개의 data chip과 2개의 parity chip을 사용하며, data chip은 실제 필요한 데이터, parity chip은 오류정정에 사용되는 데이터입니다.
// 2개의 sub-channel이 하나의 channel을 형성합니다.
// 본 오류정정부호는 x4 chip DDR5 기준 하나의 channel에서 2개의 chip error를 정정하는 것을 목표로 합니다.
// 본 코드는 1개 sub-channel에서의 시뮬레이션입니다. 1개 sub-channel에서 1개의 chip error 정정에 성공할 경우, 1개 channel에서 2개의 chip error 정정이 가능합니다. (2개의 sub-channel에 동일한 오류정정부호를 적용)
